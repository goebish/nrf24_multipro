...
static uint8_t H8_3D_checksum();
static void H8_3D_send_packet(uint8_t bind);

uint32_t proH8_3D::loop() {
...
static void H8_3D_send_packet(uint8_t bind) {
    Ppacket[0] = 0x13;
    memcpy(&Ppacket[1], H8_3D_txid, 4);
    Ppacket[8] = H8_3D_txid[0] + H8_3D_txid[1] + H8_3D_txid[2] + H8_3D_txid[3]; // txid checksum
    memset(&Ppacket[9], 0, 10);
    if(bind) {
        Ppacket[5] = 0x00;
        Ppacket[6] = 0x00;
        Ppacket[7] = 0x01;
    } else {
        Ppacket[5] = H8_3D_rf_chan;
        Ppacket[6] = 0x08;
        Ppacket[7] = 0x03;
        Ppacket[9] = multipro.getChannel(CH_THROTTLE, 0, 0xff); // throttle
        if(multipro.getChannel(CH_RUDDER) > PPM_MID) {
            Ppacket[10] = multipro.getChannel(CH_RUDDER, 0, 0x3c); // rudder
        } else {
            Ppacket[10] = multipro.getChannel(CH_RUDDER, 0x80, 0xbc); // rudder
        }
        Ppacket[11] = multipro.getChannel(CH_ELEVATOR, 0x43, 0xbb); // elevator
        Ppacket[12] = multipro.getChannel(CH_AILERON, 0xbb, 0x43); // aileron
        // neutral trims
        Ppacket[13] = 0x20;
        Ppacket[14] = 0x20;
        Ppacket[15] = 0x20;
        Ppacket[16] = 0x20;
        Ppacket[17] = H8_3D_FLAG_RATE_HIGH | GET_FLAG(CH_3WAY, H8_3D_FLAG_LED) // JJRC H22
        | GET_FLAG(CH_FLIP, H8_3D_FLAG_FLIP) | GET_FLAG(CH_HEADLESS, H8_3D_FLAG_HEADLESS) // RTH+Headless on H8 3D
        | GET_FLAG(CH_AUX6, H8_3D_FLAG_RTH); // 180/360 flip mode on H8 3D
        if(Ppacket[9] == 0x00 && Ppacket[10] >= 0xb6 && Ppacket[11] <= 0x49 && Ppacket[12] >= 0xb5) {
            Ppacket[18] |= H8_3D_FLAG_CALIBRATE;
        }
    }
    Ppacket[18] = 0x00;
    Ppacket[19] = H8_3D_checksum(); // data checksum
    ...
    XN297_WritePayload(Ppacket, H8_3D_PACKET_SIZE);
}
